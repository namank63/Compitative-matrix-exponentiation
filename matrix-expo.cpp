#include <bits/stdc++.h>
using namespace std;
void multiplyMatirix(int resMatrix[2][2], int b[2][2])          // 2 Matrix Multiplication function
{
    int temp[2][2] = {{0}};
    for(int i=0; i<2; i++)
    {
...
void matrixFastExpo(int resMatrix[2][2], int n, int x, int y)
{
    ...
    multiplyMatirix(resMatrix,resMatrix);
    if(n % 2 != 0) multiplyMatirix(resMatrix, constantMatrix);
}
...
int matrixExpo(int n, int x, int y)
{
    ...
    if(n == 1) return y;                    
    if(n == 0) return x;                    
    matrixFastExpo(constantMatrix, n-1, x, y);
    return (constantMatrix[0][0] * y) + (constantMatrix[0][1] * x);
}
int main()
{
    int x,y,n;
